  ...
  addLink: (link: Link) =>
    set((state) => ({
      site: state.site
        ? {
            ...state.site,
            links: state.site.links ? [...state.site.links, link] : [link],
          }
        : null,
      isChanged: true,
    })),
      ...
      site: state.site
        ? {
            ...
            links: state.site.links
              ? state.site.links.map((l) => (l.id === link.id ? link : l))
              : [],
          }
        : null,
      isChanged: true,
...
function AvatarField() {
  ...
  return (
    <EditorField>
      ...
      <PictureInput url={avatarUrl} setUrl={setAvatarUrl} resource="Avatar" />
    </EditorField>
  );
}
...
function BannerField() {
  ...
  return (
    <EditorField>
      ...
      <PictureInput url={bannerUrl} setUrl={setBannerUrl} resource="Banner" />
    </EditorField>
  );
}
          ...
          <div className="h-8 flex flex-row items-center justify-between">
            ...
            <div className="flex flex-row items-center h-8 flex-1">
              <input
                ...
                placeholder="0"
                value={kink.rating !== null ? kink.rating.toString() : ""}
                onChange={(e) => {
...
function EditorFieldInputInput({
  ...
  return (
    <input className={cn("flex-1 h-full outline-none", className)} {...props} />
  );
}
  ...
  return (
    <button
      className={cn(
        ...
        size === "default" && "px-4",
        size === "icon" && "size-8 min-w-8 flex items-center justify-center",
        className,
      )}
      {...props}
    >
    ...
    <div>
      <EditorFieldButtons>
        ...
        >
          {isLoading ? (
            <>
              <LoaderIcon className="animate-spin" /> Uploading {resource}...
            </>
          ) : (
            <>
  ...
  return (
    <div
      className={cn(
        ...
        !isChanged && "opacity-0 pointer-events-none -mb-12 translate-y-5 mt-0",
      )}
    >
